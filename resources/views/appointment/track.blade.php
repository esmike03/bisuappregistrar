    ...
    <div class="flex justify-center w-full md:inset-0 h-screen">
        <div class="relative p-4 w-screen max-w-md max-h-full">
            ...
            <div class="relative bg-purple-800 backdrop-filter backdrop-blur-sm bg-opacity-30 rounded-lg shadow p-2">
                <!-- Modal header -->
                <div class="bg-purple-950 flex items-center justify-between p-4 md:p-5 border-b rounded-md dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-100">
                        Appointment Status
                    </h3>
                    <p class="bg-amber-400 rounded-md px-3 py-1 text-purple-800 uppercase">
                        {{ $code->appstatus }}
                    </p>
                </div>
                ...
                <div class="p-4 md:p-5" x-slot:content>
                    ...
                    <p class="text-gray-400">Name: <span class="font-semibold text-white">{{ $code->lname }},
                            {{ $code->fname }}</span></p>
                ...
                <div class="flex justify-center text-purple-900">
                    <a href="/" @click="loading = true; fetch('/api/endpoint').then(() => loading = false)" class="hover:bg-purple-900 bg-gray-300 px-12 py-2 rounded-md mx-2 my-2 w-full text-white text-center">
                    ...
                    <a @click="loading = true; fetch('/api/endpoint').then(() => loading = false)"
                       ...
                       :class="{ ' opacity-50 cursor-not-allowed': '{{ $code->appstatus }}' !== 'PENDING' }"
